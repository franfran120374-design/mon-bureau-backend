/**
 * Module de veille scientifique — Biomimétisme
 * -----------------------------------------------
 * Ce fichier est un "router" Express à brancher sur votre backend
 * existant (mon-bureau-backend.onrender.com).
 *
 * Ce qu'il fait :
 *  - Reçoit une recherche (mot-clé) depuis le frontend du Laboratoire
 *  - Interroge, CÔTÉ SERVEUR (donc sans aucun problème de CORS),
 *    trois bases scientifiques publiques et gratuites :
 ...
 *  - Fusionne, nettoie et trie les résultats par date
 *  - Les renvoie au format JSON, prêts à afficher
 *  - Garde un petit cache en mémoire (10 minutes) pour éviter
 *    de spammer les API si plusieurs personnes/tuiles interrogent
 *    la même recherche dans un court laps de temps
 *
 * INSTALLATION (voir README.md pour le détail pas-à-pas) :
 *  1. Copiez ce fichier dans le dossier de votre backend, par ex. :
 *     routes/veille-biomimetisme.js
 *  2. Dans votre fichier principal (souvent server.js ou index.js),
 *     ajoutez ces deux lignes :
 *
 *       const veilleBiomimetisme = require('./routes/veille-biomimetisme');
 *       app.use('/api/veille/biomimetisme', veilleBiomimetisme);
 *
 *  3. Redéployez sur Render (git push suffit si le déploiement
 *     automatique est activé).
 *
 * Ce module n'a besoin d'AUCUNE clé API et d'AUCUNE variable
 * d'environnement : CrossRef, Semantic Scholar et PubMed sont en
 * accès public gratuit. (PubMed limite à 3 requêtes/seconde sans
 * clé — largement suffisant pour un usage personnel.)
 */
...
// ---- Route principale : GET /api/veille/biomimetisme/live?q=...&limite=15 ----
router.get('/live', async (requete, reponse) => {
